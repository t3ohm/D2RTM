...
namespace TokenManager.Helpers
{
    public static class AudioHelper
    {
        ...
        public static IList<ISimpleAudioVolume> GetProcessVolumeControls(Process process)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (ctl2 != null)
                {
                    ...
                    var session = new AudioSession(ctl2);
                    if (session != null && session.Process != null)
                    {
                        if (session.Process.Id == process.Id)
                        {
                            if (volume != null)
                            {
                                list.Add(volume);
        ...
        public static void IsMuted(Process process)
        {
            int id = process.Id;
        ...
        public static void UnMuteProcess(Process process)
        {
            int id = process.Id;
            foreach (ISimpleAudioVolume ctrl in GetProcessVolumeControls(process))
            {
                Guid guid = Guid.Empty;
                ctrl.SetMute(false, guid);
            }
        }
        public static void MuteProcess(Process process)
        {
            int id = process.Id;
            foreach (ISimpleAudioVolume ctrl in GetProcessVolumeControls(process))
            {
